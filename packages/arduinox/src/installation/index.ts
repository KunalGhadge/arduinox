...
declare global {
  const ARDUINOX_VERSION: string
  const ARDUINOX_CHANNEL: string
}
...
export namespace Installation {
  ...
  export async function method() {
    if (process.execPath.includes(path.join(".arduinox", "bin"))) return "curl"
    ...
    for (const check of checks) {
      ...
      const installedName =
        check.name === "brew" || check.name === "choco" || check.name === "scoop" ? "arduinox" : "arduinox-ai"
  ...
  export async function upgrade(method: Method, target: string) {
    ...
    switch (method) {
      case "curl":
        cmd = $`curl -fsSL https://arduinox.ai/install | bash`.env({
          ...process.env,
          VERSION: target,
        })
        break
  ...
  export const VERSION = typeof ARDUINOX_VERSION === "string" ? ARDUINOX_VERSION : "local"
  export const CHANNEL = typeof ARDUINOX_CHANNEL === "string" ? ARDUINOX_CHANNEL : "local"
  export const USER_AGENT = `arduinox/${CHANNEL}/${VERSION}/${Flag.OPENCODE_CLIENT}`
