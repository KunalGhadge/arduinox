...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for ArduinoX.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal ArduinoX sessions
...
ArduinoX acp --cwd /path/to/project
```
...
```bash
ArduinoX_ENABLE_QUESTION_TOOL=1 ArduinoX acp
```
...
- **Terminal Integration**: Full terminal support via ArduinoX's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | ArduinoX acp
```
...
ACP sessions map cleanly to ArduinoX's internal session model:
