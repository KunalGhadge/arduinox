...
import { NamedError } from "@arduinox-ai/util/error"
...
export namespace Config {
  ...
  export function managedConfigDir() {
    return process.env.ARDUINOX_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
  }
  ...
  export const state = Instance.state(async () => {
    ...
    // Config loading order (low -> high precedence): https://arduinox.ai/docs/config#precedence-order
    ...
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        ...
        if (!remoteConfig.$schema) remoteConfig.$schema = "https://arduinox.ai/config.json"
    ...
    if (process.env.ARDUINOX_CONFIG_CONTENT) {
      result = mergeConfigConcatArrays(
        result,
        await load(process.env.ARDUINOX_CONFIG_CONTENT, {
  ...
  export async function installDependencies(dir: string) {
    ...
    json.dependencies = {
      ...json.dependencies,
      "@arduinox-ai/plugin": targetVersion,
    }
  ...
  export async function needsInstall(dir: string) {
    ...
    const depVersion = dependencies["@arduinox-ai/plugin"]
    if (!depVersion) return true

    const targetVersion = Installation.isLocal() ? "latest" : Installation.VERSION
    if (targetVersion === "latest") {
      const isOutdated = await PackageRegistry.isOutdated("@arduinox-ai/plugin", depVersion, dir)
      if (!isOutdated) return false
      log.info("Cached version is outdated, proceeding with install", {
        pkg: "@arduinox-ai/plugin",
  ...
  export const Info = z
    .object({
      ...
      command: z
        ...
        .optional()
        .describe("Command configuration, see https://arduinox.ai/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://arduinox.ai/docs/agents"),
      provider: z
    ...
    if (existsSync(legacy)) {
      await import(pathToFileURL(legacy).href, {
        ...
        .then(async (mod) => {
          ...
          result["$schema"] = "https://arduinox.ai/config.json"
  ...
  async function load(text: string, options: { path: string } | { dir: string; source: string }) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema && isFile) {
        parsed.data.$schema = "https://arduinox.ai/config.json"
        const updated = original.replace(/^\s*\{/, '{\n  "$schema": "https://arduinox.ai/config.json",')
