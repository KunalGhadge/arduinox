...
const managedConfigDir = process.env.ARDUINOX_TEST_MANAGED_CONFIG_DIR!
...
test("does not try to install dependencies in read-only OPENCODE_CONFIG_DIR", async () => {
  ...
  const prev = process.env.ARDUINOX_CONFIG_DIR
  process.env.ARDUINOX_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.ARDUINOX_CONFIG_DIR
    else process.env.ARDUINOX_CONFIG_DIR = prev
  }
})
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "opencode.json"),
        JSON.stringify({ $schema: "https://arduinox.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
