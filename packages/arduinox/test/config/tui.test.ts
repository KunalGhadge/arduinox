...
afterEach(async () => {
  delete process.env.ARDUINOX_CONFIG
  delete process.env.ARDUINOX_TUI_CONFIG
...
test("project config takes precedence over OPENCODE_TUI_CONFIG (matches OPENCODE_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.ARDUINOX_TUI_CONFIG = custom
    },
  })
...
test("does not derive tui path from OPENCODE_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.ARDUINOX_CONFIG = path.join(customDir, "opencode.json")
    },
  })
